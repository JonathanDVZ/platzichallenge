describe('test home screen', () => {
  ...
  it('characters items by default', () => {
    cy.get('.ptz-characters-section__result .ptz-character-item').should('have.length', 0);
  });
  ...
  it('make a search', () => {
    ...
    cy.get('.ptz-characters-section__result .ptz-character-item').should('not.equal', totalResult);
  });
  ...
  it('make an empty search', () => {
    ...
    cy.wait('@getCharacters');
    cy.get('.ptz-search-box form').submit();
    cy.get('.ptz-characters-section__result .ptz-character-item').its('length').as('searchResult');
  ...
  it('select favorite characters', () => {
    ...
    cy.get('.ptz-characters-section__result .ptz-character-item').should('have.length', 2);
    ...
    cy.get('.ptz-characters-section__result .ptz-character-item').should('have.length', 4);
  });
});
